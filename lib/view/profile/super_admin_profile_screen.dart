...
class SuperAdminProfileScreen extends StatefulWidget {
  const SuperAdminProfileScreen(
      {Key? key, this.userModel, this.companyModel, required this.role, this.superAdminModel})
      : super(key: key);

  final UserModel? userModel;
  final CompanyModel? companyModel;
  final SuperAdminModel? superAdminModel;

  final String role;

  @override
  SuperAdminProfileScreenState createState() => SuperAdminProfileScreenState();
}

class SuperAdminProfileScreenState extends State<SuperAdminProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    print("superAdminModel: ${widget.superAdminModel.toString()}");
      ...
      body: Stack(
        children: [
          ListView(
            ...
            children: [
              ..._buildHeader((widget.role == "Super Admin")
                  ? widget.superAdminModel
                  : widget.companyModel),
              if (widget.role == "Super Admin") ..._buildSuperAdminFields(),
  ...
  Widget _buildCoverImage(dynamic model) {
    final image =
    (widget.role == "Super Admin") ? model.userAvatar : model.coverImage;
    return SizedBox(
      ...
      child: Stack(
        children: [
          ...
          //     child: Image.file(File(_logoImage!), fit: BoxFit.fill),
          //   ),
          // )
          //     : Container(
          //   height: Utils.scrHeight * .18,
          ...
          //         ? CachedNetworkImage(imageUrl:model.userAvatar!, fit: BoxFit.fill)
          //         : Image.asset('assets/images/profile.png'),
          //   ),
          // ),
          // Positioned(
  ...
  void setUserDataToTextFields() {
    ...
    if (widget.role == "Super Admin") {
      _usernameController.text = widget.superAdminModel!.username;
      _nameController.text = widget.superAdminModel?.name ?? '';
      _phoneNumberController.text = widget.superAdminModel?.phoneNumber ?? '';
      _companyEmailController.text = widget.superAdminModel?.email ?? '';
    }
  ...
  Future<void> _updateProfile() async {
    ...
    try {
      if (widget.role == "Super Admin") {
        model = setUpdatedSuperAdminModel(widget.superAdminModel!);
      } else {
  ...
  Future<void> _updateProfileImage(String field, String imagePath) async {
    ...
    try {
      ...
      await ProfileService.updateSuperAdminProfilePicture(
  ...
  SuperAdminModel setUpdatedSuperAdminModel(SuperAdminModel superAdminModel) {
    // Get updated values from text controllers
    String username = _usernameController.text.trim();
    String name = _nameController.text.trim();
    String phoneNumber = _phoneNumberController.text.trim();
    ...
    return SuperAdminModel(
        ...
        name: name,
        phoneNumber: phoneNumber,
        email: superAdminModel.email,
        password: superAdminModel.password,
        role: superAdminModel.role,
        userAvatar: superAdminModel.userAvatar,
        coverImage: superAdminModel.coverImage,
        userAvatarName: superAdminModel.userAvatarName,
        coverImageName: superAdminModel.coverImageName);
  }
  ...
  List<Widget> _buildSuperAdminFields() => [
